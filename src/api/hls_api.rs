...
use crate::processing::hls_parser::{rewrite_hls, M3U_HLSR_PREFIX};
use crate::{try_option_bad_request, try_result_bad_request};
...
pub(in crate::api) async fn handle_hls_stream_request(app_state: &Data<AppState>, user: &ProxyUserCredentials, pli: &dyn PlaylistEntry, input: &ConfigInput, target_type: TargetType) -> HttpResponse {
    ...
    match request_utils::download_text_content(Arc::clone(&app_state.http_client), input, &url, None).await {
        Ok(content) => {
