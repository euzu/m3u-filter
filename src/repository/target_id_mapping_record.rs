 ...
 */
pub(in crate::repository) struct TargetIdMappingRecord {
    pub provider_id: u32,
    pub virtual_id: u32,
    pub uuid: [u8; 32],
}
...
impl TargetIdMappingRecord {
    fn to_bytes(&self) -> [u8; 40] {
        ...
        let mut combined_bytes: [u8; 40] = [0; 40];
        combined_bytes[..4].copy_from_slice(&provider_id_bytes);
        combined_bytes[4..].copy_from_slice(&virtual_id_bytes);
        combined_bytes[8..].copy_from_slice(&self.uuid);
        combined_bytes
    }
}

pub(in crate::repository) struct TargetIdMapping {
    dirty: bool,
...
impl TargetIdMapping {
    pub(crate) fn insert_entry(&mut self, provider_id: u32, uuid: [u8; 32]) -> u32 {
        self.dirty = true;
        self.virtual_id_counter += 1;
        self.insert(TargetIdMappingRecord { provider_id, virtual_id: self.virtual_id_counter, uuid });
        self.virtual_id_counter
    }

    fn new(records: Vec<Rc<TargetIdMappingRecord>>) -> Self {
    ...
    fn get_by_provider_id(&self, provider_id: u32) -> Option<&Rc<TargetIdMappingRecord>> {
        self.by_provider_id.get(&provider_id)
    }

    fn get_by_virtual_id(&self, virtual_id: u32) -> Option<&Rc<TargetIdMappingRecord>> {
        self.by_virtual_id.get(&virtual_id)
    }

    fn get_by_uuid(&self, uuid: &[u8; 32]) -> Option<&Rc<TargetIdMappingRecord>> {
    ...
    pub fn from_path(path: &Path) -> Self {
        match file_utils::open_file_append(path, false) {
            Ok(mut file) => TargetIdMapping::from_file(&mut file),
            _ => TargetIdMapping::new(vec![])
        }
    }
    ...
    pub fn from_file(file: &mut File) -> Self {
        ...
        if let Ok(_) = file.seek(SeekFrom::Start(0)) {
            loop {
                ...
                let mut virtual_id_bytes = [0u8; 4];
                let mut uuid = [0u8; 32];
                if let Err(_) = file.read_exact(&mut provider_id_bytes) {
                    break;
                }
                if let Err(_) = file.read_exact(&mut virtual_id_bytes) {
                    break;
                }
                if let Err(_) = file.read_exact(&mut uuid) {
                    break;
                }
                let provider_id = u32::from_le_bytes(provider_id_bytes);
                let virtual_id = u32::from_le_bytes(virtual_id_bytes);
                records.push(Rc::new(TargetIdMappingRecord { provider_id, virtual_id, uuid }));
            }
        }
        TargetIdMapping::new(records)
    }
}